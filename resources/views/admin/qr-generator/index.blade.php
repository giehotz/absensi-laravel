...
@section('title', 'Daftar QR Code - Admin')
...
<div class="space-y-6 max-w-7xl mx-auto pb-12">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-2">
                <span class="px-2.5 py-0.5 bg-[#FFD43B] text-black font-heading font-black text-[10px] uppercase rounded border-2 border-black shadow-[2px_2px_0px_#000]">
                    Utilitas Admin
                </span>
                <span class="text-xs text-slate-500 font-semibold">{{ $qrCodes->total() }} Total QR Code</span>
            </div>
            <h1 class="font-heading font-black text-2xl lg:text-3xl text-slate-900 tracking-tight mt-1">
                QR Generator
            </h1>
            <p class="text-xs font-semibold text-slate-500 mt-1">
                Kelola dan pantau seluruh QR Code yang dibuat oleh Administrator dan Guru.
            </p>
        </div>

        <div>
            <a href="{{ route('admin.qr-generator.create') }}" 
    ...
    <div class="bg-white border-3 border-black rounded-2xl p-4 sm:p-5 shadow-[4px_4px_0px_0px_#000000]">
        <form action="{{ route('admin.qr-generator.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-12 gap-3">
            {{-- Input Pencarian --}}
            ...
            <div class="sm:col-span-4">
                <select name="user_id" 
                        class="w-full px-3.5 py-2 bg-slate-50 border-2 border-black rounded-xl text-xs font-semibold text-slate-800 shadow-[2px_2px_0px_#000] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#5294FF] cursor-pointer">
                    <option value="">Semua Pembuat</option>
                    @foreach($creators as $creator)
                        <option value="{{ $creator->id }}" {{ request('user_id') == $creator->id ? 'selected' : '' }}>
                            {{ $creator->name }} ({{ ucfirst($creator->role) }})
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="sm:col-span-2 flex items-center gap-2">
                <button type="submit" 
                        class="flex-1 py-2 px-3 bg-[#FFD43B] hover:bg-yellow-400 text-black font-heading font-black text-xs uppercase tracking-wider rounded-xl border-2 border-black shadow-[2px_2px_0px_#000] hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none transition-all cursor-pointer text-center">
                    Cari
                </button>
                @if(request()->hasAny(['search', 'user_id']))
                    <a href="{{ route('admin.qr-generator.index') }}" 
    ...
    @if($qrCodes->isEmpty())
        <div class="bg-white border-3 border-black rounded-2xl p-12 text-center shadow-[5px_5px_0px_0px_#000000]">
            ...
            <p class="text-xs text-slate-500 max-w-sm mx-auto mb-5 font-medium">
                @if(request()->hasAny(['search', 'user_id']))
                    Tidak ada QR Code yang sesuai dengan filter pencarian Anda.
                @else
                    Belum ada QR Code yang dibuat. Klik tombol di bawah untuk membuat QR Code pertama Anda.
                @endif
            </p>
            <a href="{{ route('admin.qr-generator.create') }}" 
                ...
                <div class="bg-white border-3 border-black rounded-2xl p-4 shadow-[4px_4px_0px_0px_#000000] hover:shadow-[6px_6px_0px_0px_#000000] hover:-translate-y-1 transition-all flex flex-col justify-between group">
                    <div>
                        ...
                        <div class="mt-2.5 pt-2.5 border-t border-slate-100 flex items-center justify-between text-[11px]">
                            ...
                            <span class="font-heading font-bold text-slate-800 truncate max-w-[130px]">
                                {{ $qr->user?->name ?? 'Admin' }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 pt-3 border-t-2 border-slate-100 space-y-2">
                        <div class="grid grid-cols-2 gap-2">
                            <a href="{{ route('admin.qr-generator.download', ['qrGenerator' => $qr, 'format' => 'png']) }}" 
                            ...
                            <a href="{{ route('admin.qr-generator.download', ['qrGenerator' => $qr, 'format' => 'svg']) }}" 
                        ...
                        <div class="flex items-center gap-2">
                            <a href="{{ route('admin.qr-generator.show', $qr) }}" 
                            ...
                            <form action="{{ route('admin.qr-generator.destroy', $qr) }}" method="POST" onsubmit="return confirmDelete(event, this)">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
