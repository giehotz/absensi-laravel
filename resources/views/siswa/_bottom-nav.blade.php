...
<!-- ========================================================================= -->
@php
    $isSavingsPage = request()->routeIs('siswa.savings.*');
@endphp
<nav id="floatingBottomNav" class="fixed bottom-3 sm:bottom-5 left-1/2 -translate-x-1/2 z-40 w-[94%] max-w-md bg-white border-3 border-black rounded-2xl p-1.5 sm:p-2 shadow-[5px_5px_0px_0px_#000] transition-all">
    <div class="flex items-center justify-between gap-1 sm:gap-1.5">
        <!-- 1. Beranda -->
        @if($isSavingsPage)
            <a href="{{ route('siswa.dashboard') }}#tab=beranda" id="navBtn-beranda" 
                class="nav-tab-btn group relative p-2 sm:p-2.5 rounded-xl transition-all text-slate-600 hover:text-black border-2 border-transparent hover:bg-slate-100 hover:scale-105"
        ...
        <!-- 2. QR Presensi -->
        @if($isSavingsPage)
            <a href="{{ route('siswa.dashboard') }}#tab=qr" id="navBtn-qr" 
                class="nav-tab-btn group relative p-2 sm:p-2.5 rounded-xl transition-all text-slate-600 hover:text-black border-2 border-transparent hover:bg-slate-100 hover:scale-105"
        ...
        <!-- 3. Jadwal -->
        @if($isSavingsPage)
            <a href="{{ route('siswa.dashboard') }}#tab=jadwal" id="navBtn-jadwal" 
                class="nav-tab-btn group relative p-2 sm:p-2.5 rounded-xl transition-all text-slate-600 hover:text-black border-2 border-transparent hover:bg-slate-100 hover:scale-105"
        ...
        <!-- 4. Izin -->
        @if($isSavingsPage)
            <a href="{{ route('siswa.dashboard') }}#tab=izin" id="navBtn-izin" 
                class="nav-tab-btn group relative p-2 sm:p-2.5 rounded-xl transition-all text-slate-600 hover:text-black border-2 border-transparent hover:bg-slate-100 hover:scale-105"
        ...
        <!-- 5. Riwayat -->
        @if($isSavingsPage)
            <a href="{{ route('siswa.dashboard') }}#tab=riwayat" id="navBtn-riwayat" 
                class="nav-tab-btn group relative p-2 sm:p-2.5 rounded-xl transition-all text-slate-600 hover:text-black border-2 border-transparent hover:bg-slate-100 hover:scale-105"
        ...
        <!-- 6. Tabungan (Link Mandiri ke /siswa/tabungan) -->
        <a href="{{ route('siswa.savings.index') }}" id="navBtn-tabungan" 
            class="nav-tab-btn group relative p-2 sm:p-2.5 rounded-xl transition-all {{ $isSavingsPage ? 'text-black bg-[#FFD43B] border-2 border-black shadow-[2px_2px_0px_0px_#000]' : 'text-slate-600 hover:text-black border-2 border-transparent hover:bg-slate-100' }} hover:scale-105"
