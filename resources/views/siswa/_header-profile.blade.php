...
<div class="bg-white neo-box p-4 flex items-center justify-between gap-3 relative overflow-hidden">
    <div class="flex items-center gap-3 min-w-0">
        ...
        @if(!empty($student->photo) && \Illuminate\Support\Facades\Storage::disk('public')->exists($student->photo))
            <img src="{{ asset('storage/' . $student->photo) }}" alt="Foto {{ $student->user->name }}" class="w-13 h-13 rounded-full border-2 border-black object-cover shrink-0 shadow-[2px_2px_0px_0px_#000]">
        @else
            <div class="w-13 h-13 rounded-full bg-[#5294FF] border-2 border-black flex items-center justify-center font-heading font-black text-xl text-white shrink-0 shadow-[2px_2px_0px_0px_#000]">
        ...
        <div class="min-w-0">
            ...
            <h1 class="font-heading font-black text-base sm:text-lg text-black truncate mt-1 leading-tight">
                {{ $student->user->name }}
            </h1>
    ...
    <button onclick="openModal('modalFullscreenQr')" class="neo-btn bg-[#FFD43B] hover:bg-yellow-400 p-2.5 flex flex-col items-center justify-center shrink-0" title="Tampilkan QR Presensi Layar Penuh">
