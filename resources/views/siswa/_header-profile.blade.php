...
<div class="bg-white neo-box p-4 flex items-center justify-between gap-3 relative overflow-hidden">
    <div onclick="switchTab('profil')" class="flex items-center gap-3 min-w-0 cursor-pointer group" title="Buka Profil Lengkap Siswa">
        ...
        @if($student->photo_url)
            <img src="{{ $student->photo_url }}" alt="Foto {{ $student->user->name }}" class="w-13 h-13 rounded-full border-2 border-black object-cover shrink-0 shadow-[2px_2px_0px_0px_#000] group-hover:scale-105 transition-transform">
        @else
    ...
    <button type="button" onclick="openModal('modalFullscreenQr')" class="neo-btn bg-[#FFD43B] hover:bg-yellow-400 p-2.5 flex flex-col items-center justify-center shrink-0 cursor-pointer" title="Tampilkan QR Presensi Layar Penuh">
